class URLs {
    ...
    static ADD_USER_TO_MATCH(matchName, userEmail, matchPassword) {
        if (matchPassword) {
            return this.DEV_SERVER + `/websockets/REST/matches/add-user-to-match?matchName=${matchName}&userEmail=${userEmail}&matchPassword=${matchPassword}`;
        }
        return this.DEV_SERVER + `/websockets/REST/matches/add-user-to-match?matchName=${matchName}&userEmail=${userEmail}`;
    }

    static ADD_MATCH(userEmail, matchPassword) {
        if (matchPassword) {
            return this.DEV_SERVER + `/websockets/REST/matches/add-match?userEmail=${userEmail}&matchPassword=${matchPassword}`;
        }
